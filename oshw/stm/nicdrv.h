...
#define htons(x) ( (((u16_t)(x))>>8) | (((x)&0xFF)<<8) )
#define ntohs(x) ( (((u16_t)(x))>>8) | (((x)&0xFF)<<8) )
#define htonl(x) ( (((u32_t)(x))>>24) | (((x)&0xFF0000)>>8) | (((x)&0xFF00)<<8) | (((x)&0xFF)<<24) )
...
typedef unsigned char stm_mutex_t;
...
typedef struct
{
   ...
   stm_mutex_t getindex_mutex;
   stm_mutex_t tx_mutex;
   stm_mutex_t rx_mutex;
} ecx_portt;
